    ...
    try {
      if (user) {
        ...
        unsubscribe = client.subscribe(channel, (response) => {
          ...
          const updatedDoc = response.payload as Partial<Transaction> & { $id: string, userId: string };

          if (updatedDoc.userId !== user.$id) return;

          if (eventType.includes(".create")) {
            setTransactions((prev) => {
              if (prev.find((t) => t.$id === updatedDoc.$id)) return prev;
              
              const normalizedType = (updatedDoc.transaction_type || "").toLowerCase();
              return [
                {
                  $id: updatedDoc.$id,
                  transaction_amount: Number(updatedDoc.transaction_amount || 0),
                  transaction_created: updatedDoc.transaction_created || new Date().toISOString(),
                  transaction_type: normalizedType,
                  transaction_description: updatedDoc.transaction_description,
                  transaction_status: updatedDoc.transaction_status || (normalizedType === "debit" ? "Pending" : "Complete"),
                  transaction_file_id: updatedDoc.transaction_file_id,
                },
                ...prev,
              ];
            });
            ...
            setTransactions((prev) =>
              prev.map((txn) =>
                txn.$id === updatedDoc.$id
                  ? {
                      ...txn,
                      transaction_status: updatedDoc.transaction_status || txn.transaction_status,
                      transaction_file_id: updatedDoc.transaction_file_id || txn.transaction_file_id,
                    }
                  : txn,
              ),
            );
